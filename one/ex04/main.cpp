 ...
 * @return A string containing the entire content of the file, with lines separated by newline characters.
 * @note If the file is empty, an empty string is returned.
 * @note The function does not close the file stream; it is the caller's responsibility to manage the file stream's lifecycle.
 * @warning Ensure that the file stream is valid and opened before calling this function to avoid undefined behavior.
 */
 ...
 * @note If s1 is an empty string, no replacements will be made and the original content is returned.
 * @note If s1 is not found in content, no replacements will be made and the original content is returned.
